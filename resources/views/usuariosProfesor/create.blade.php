...
<div class="container">
    ...
    <form action="{{ route('usuarioProfesor.store') }}" method="POST">
        ...
        <!--div class="mb-3">
            ...
            <input type="password" class="form-control" id="contrasenia" name="contrasenia" value="{{ old('contrasenia') }}" required>
        </div-->
        ...
        <div class="mb-3">
            ...
            <select class="form-select" id="programaEducativo" name="programaEducativo" required>
                ...
                @foreach($programas as $programa)
                    <option value="{{ $programa->idProgramaEducativo }}">{{ $programa->nombre }}, Nivel = "{{ $programa->nivel }}"</option>
                @endforeach
            </select>
        </div>

        <button type="submit" class="btn colorButton">Crear alumno</button>

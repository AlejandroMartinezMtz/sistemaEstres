...
<div class="container">
    <div class="row my-4">
        <div class="col-md-6">
            <h2>Alumnos</h2>
            <h5>Profesor / <spand>Alumnos</spand></h5>
        </div>
        <div class="col-md-6 text-end">
            <a href="{{ route('usuarioProfesor.create')}}" class="btn colorButton"><i class="bi bi-plus icon-large"></i>
            Agregar</a>
        </div>
    </div>
    ...
    <div class="row mb-4">
    <div class="col-md-4">
        <form action="{{ route('usuarioProfesor.index') }}" method="GET" id="searchForm">
                ...
                @else
                    @foreach($alumnos as $usuario)
                        <tr>
                            ...
                            <td>
                                <a href="{{ route('usuarioProfesor.edit', $usuario->idUsuario) }}" class="btn btn-sm btn-custom-edit"><i class="bi bi-pencil"></i> Editar</a>
                                <form action="{{ route('usuarioProfesor.destroy', $usuario->idUsuario) }}" method="POST" style="display: inline-block;" onsubmit="return confirm('“¿Está seguro de eliminar este registro? Nota: El estado del usuario cambia de activo a inactivo”.');">
                                    @csrf
                                    @method('DELETE')

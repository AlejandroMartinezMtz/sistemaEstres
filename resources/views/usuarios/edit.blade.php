...
<div class="container">
    ...
    <form action="{{ route('usuarios.update', $usuario->idUsuario) }}" method="POST">
            ...
            <select class="form-select" id="tipoUsuario" name="tipoUsuario" required>
                <option value="">Seleccionar tipo de usuario</option>
                    @foreach($tipos as $tipo)
                        <option value="{{ $tipo->idTipo_usuario }}" {{ $usuario->fk_tipo_usuario == $tipo->idTipo_usuario ? 'selected' : '' }}>
                    {{ $tipo->tipo }}
                </option>
                    @endforeach
            </select>
        ...
        <button type="submit" class="btn colorButton">Actualizar usuario</button>
